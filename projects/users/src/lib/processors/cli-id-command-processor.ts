...
export class CliIdCommandProcessor implements ICliCommandProcessor {
    ...
    async processCommand(command: CliProcessCommand, context: ICliExecutionContext): Promise<void> {
        ...
        context.writer.writeln(`uid=${user.id} name=${user.name} groups=${groups}`);
    }
    ...
    writeDescription(context: ICliExecutionContext): void {
        ...
        writer.writeln();
        writer.writeln(`  ${writer.wrapInColor('id', CliForegroundColor.Cyan)}                    Show current user`);
        writer.writeln(`  ${writer.wrapInColor('id <username>', CliForegroundColor.Cyan)}          Show specific user`);
    }
}
